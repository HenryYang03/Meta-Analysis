...
```{r}
postscript("time_spent_in_target_quadrant_forest_plot_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
...
```{r}
find.outliers(meta_analysis_time_spend_in_target_quadrant)
```

```{r}
meta_analysis_time_spend_in_target_quadrant.inf <- InfluenceAnalysis(meta_analysis_time_spend_in_target_quadrant, random = TRUE)

plot(meta_analysis_time_spend_in_target_quadrant.inf, "baujat")
plot(meta_analysis_time_spend_in_target_quadrant.inf, "es")
plot(meta_analysis_time_spend_in_target_quadrant.inf, "i2")
```

```{r}
setEPS()
postscript("time_spent_I2.eps",
...
```{r}
smd_data_filtered <- subset(smd_data_time_spend_in_target_quadrant, paper_id != 18)
```

```{r}
smd_data_filtered$acupoint_group <- ifelse(
  grepl("ST", smd_data_filtered$acupoint_treatment),
  "ST",
  "GV"
)

table(smd_data_filtered$acupoint_group)
```

```{r}
meta_analysis_time_spend_in_target_quadrant_filtered <- metagen(
  smd,
  se_smd,
  data = smd_data_filtered,
  studlab = paste(paper_id, treatment_group, as.character(`age(month)`), acupoint_treatment, sep = " | "),
  sm = "SMD",
  method.tau = "DL",
  common = FALSE,
...
meta_analysis_time_spend_in_target_quadrant_filtered
```

```{r}
postscript("filtered_time_spent_in_target_quadrant_forest_plot_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
forest(meta_analysis_time_spend_in_target_quadrant_filtered,
       studlab = TRUE,
       textpos = 4,
       leftcols = c("paper_id", "treatment_group", "age(month)", "acupoint_treatment", "effect", "ci"),
       leftlabs = c("Paper id", "Control Group", "Age (month)", "Acupoint", "SMD", "95% CI"),
...
```{r}
update(meta_analysis_time_spend_in_target_quadrant_filtered,subgroup = acupoint_group,tau.common = FALSE)
```

```{r}
postscript("ST_time_spend_in_target_quadrant_filtered_forest_plot_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
forest(update(meta_analysis_time_spend_in_target_quadrant_filtered, 
...
postscript("GV_time_spend_in_target_quadrant_filtered_forest_plot_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
forest(update(meta_analysis_time_spend_in_target_quadrant_filtered, 
...
postscript("time_spent_in_target_quadrant_network_forest_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
...
```{r}
cairo_ps("time_spent_in_target_quadrnat_network_plot.eps")
