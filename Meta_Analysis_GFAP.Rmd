...
```{r}
postscript("GFAP_forest_plot_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
forest(meta_analysis_GFAP,
       studlab = TRUE,
       textpos = 4,
       leftcols = c("paper_id", "treatment_group", "age(month)", "acupoint_treatment", "area", "effect", "ci"),
...
```{r}
find.outliers(meta_analysis_GFAP)
```

```{r}
meta_analysis_GFAP.inf <- InfluenceAnalysis(meta_analysis_GFAP, random = TRUE)

plot(meta_analysis_GFAP.inf, "baujat")
plot(meta_analysis_GFAP.inf, "es")
plot(meta_analysis_GFAP.inf, "i2")
```
...
postscript("GFAP_network_forest_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
...
```{r}
cairo_ps("GFAP_network_plot.eps")
