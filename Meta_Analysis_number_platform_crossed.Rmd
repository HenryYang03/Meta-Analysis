...
```{r}
smd_data_number_platform_crossed$acupoint_group <- ifelse(
  grepl("ST", smd_data_number_platform_crossed$acupoint_treatment),
  "ST",
  "GV"
)

table(smd_data_number_platform_crossed$acupoint_group)
```

```{r}
smd_data_number_platform_crossed_GV <- smd_data_number_platform_crossed[smd_data_number_platform_crossed$acupoint_group == "GV", ]

smd_data_number_platform_crossed_GV
```


```{r}
smd_data_number_platform_crossed_GV$model_group <- ifelse(
  grepl("5xFAD", smd_data_number_platform_crossed_GV$treatment_group),
  "5xFAD",
  "APP/PS1 and cDKO"
)
...
```{r}
postscript("number_of_platfrom_crossed_forest_plot_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
forest(meta_analysis_number_platform_crossed,
       studlab = TRUE,
       textpos = 4,
       leftcols = c("paper_id", "treatment_group", "age(month)", "acupoint_treatment", "effect", "ci"),
...
```{r}
meta_analysis_number_platform_crossed_GV <- metagen(
  smd, 
  se_smd, 
  data = smd_data_number_platform_crossed_GV,
  studlab = paste(paper_id, treatment_group, as.character(`age(month)`), acupoint_treatment, sep = "  |  "),
  sm = "SMD",
  method.tau = "DL",   
  comb.fixed = FALSE,  
...
summary(meta_analysis_number_platform_crossed_GV)
```

##### Outliers

```{r}
find.outliers(meta_analysis_number_platform_crossed)
```

```{r}
meta_analysis_number_platform_crossed.inf <- InfluenceAnalysis(meta_analysis_number_platform_crossed, random = TRUE)

plot(meta_analysis_number_platform_crossed.inf, "baujat")
plot(meta_analysis_number_platform_crossed.inf, "es")
plot(meta_analysis_number_platform_crossed.inf, "i2")
```
...
```{r}
update(meta_analysis_number_platform_crossed,subgroup = acupoint_group,tau.common = FALSE)
```

```{r}
postscript("ST_number_platform_corssed_forest_plot_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
forest(update(meta_analysis_number_platform_crossed, 
                   subset = (acupoint_group == "ST")),
       studlab = TRUE,
       textpos = 4,
       leftcols = c("paper_id", "treatment_group", "age(month)", "acupoint_treatment", "effect", "ci"),
       leftlabs = c("Paper id", "Control Group", "Age (month)", "Acupoint", "SMD", "95% CI"),
...
postscript("GV_number_platform_corssed_forest_plot_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
forest(update(meta_analysis_number_platform_crossed, 
                   subset = (acupoint_group == "GV")),
       studlab = TRUE,
       textpos = 4,
       leftcols = c("paper_id", "treatment_group", "age(month)", "acupoint_treatment","effect", "ci"),
       leftlabs = c("Paper id", "Control Group", "Age (month)", "Acupoint", "SMD", "95% CI"),
...
```{r}
update(meta_analysis_number_platform_crossed_GV,subgroup = model_group,tau.common = FALSE)
```

```{r}
postscript("GV_5xFAD_number_platform_corssed_forest_plot_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
forest(update(meta_analysis_number_platform_crossed_GV, 
                   subset = (model_group == "5xFAD")),
       studlab = TRUE,
       textpos = 4,
       leftcols = c("paper_id", "treatment_group", "age(month)", "acupoint_treatment", "effect", "ci"),
       leftlabs = c("Paper id", "Control Group", "Age (month)", "Acupoint", "SMD", "95% CI"),
...
postscript("GV_APP_PS1_number_platform_corssed_forest_plot_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
forest(update(meta_analysis_number_platform_crossed_GV, 
                   subset = (model_group == "APP/PS1 and cDKO")),
       studlab = TRUE,
       textpos = 4,
       leftcols = c("paper_id", "treatment_group", "age(month)", "acupoint_treatment","effect", "ci"),
       leftlabs = c("Paper id", "Control Group", "Age (month)", "Acupoint", "SMD", "95% CI"),
...
postscript("number_platform_crossed_network_forest_output.eps", width = 15, height = 15, horizontal = FALSE, onefile = FALSE, paper = "special")
...
```{r}
cairo_ps("number_platform_crossed_network_plot.eps")
